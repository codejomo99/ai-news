...
class NewsCrawlerTest {
	...
	@Test
	@DisplayName("한겨레 크롤링 성공")
	void fetchHani_정상작동_확인() throws Exception {
		// given
		NewsCrawler crawler = new NewsCrawler();
		// when
		List<NewsArticle> articles = crawler.fetchHaniNews();
		// then
		assertThat(articles).isNotEmpty();
		for (NewsArticle article : articles) {
			System.out.println("📰 제목: " + article.getTitle());
	...
	@Test
	@DisplayName("서울뉴스 크롤링 성공")
	void fetchSeoul_정상작동_확인() throws Exception {
		// given
		NewsCrawler crawler = new NewsCrawler();
		// when
		List<NewsArticle> articles = crawler.fetchSeoulNews();
		// then
		assertThat(articles).isNotEmpty();
		for (NewsArticle article : articles) {
			System.out.println("📰 제목: " + article.getTitle());
